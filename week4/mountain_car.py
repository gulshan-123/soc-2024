...
class QAgent:
    ...
    def get_state_index(self, state):
        ...
        '''
        norm_state=(state-self.observation_space_low)/(self.observation_space_high-self.observation_space_low)
        return tuple((norm_state*np.array(self.discrete_sizes)).astype('int'))
    ...
    def update(self, state, action, reward, next_state, is_terminal):
        ...
        '''
        state1=self.get_state_index(state)
        next_state1=self.get_state_index(next_state)
        td_target=None
        if is_terminal:
            td_target=reward
        else:
    ...
    def get_action(self):    
        ...
        '''
        state1=self.get_state_index(self.state)
        if np.random.random() > self.epsilon:
            return np.argmax(self.q_table[state1])
        else:
    ...
    def agent_eval(self):
        ...
        while not done:
            state1=self.get_state_index(eval_state)
            action = np.argmax(self.q_table[state1]) # Take action based on greedy strategy now
